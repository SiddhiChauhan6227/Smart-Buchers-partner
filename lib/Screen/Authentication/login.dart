import 'dart:async';
import 'dart:io';
import 'package:erestro/Helper/keyboardOverlay.dart';
import 'package:erestro/Screen/Authentication/restorentRegistration.dart';
import 'package:erestro/Screen/Authentication/send_otp.dart';
...
print("trfgbhynjmkl, $error");
        if (!error) {
          ...
          prefs.setString(MobileFromUSer, mobile!);
          saveUserDetail(id!, username!, email!, mobile!, address!, srorename!, storeDesc!, accNo!, accname!, bankCode ?? "", bankName ?? "",
              latitutute ?? "", longitude ?? "", taxname ?? "", taxNumber!, panNumber!, status!, storeLogo!, cityid, cityname, jwtToken!);
        ...
        } else if (getdata['message']=="Your account is not yet approved.") {
          setSnackbarForLogin(scaffoldMessengerKey, context, getdata['message']);
          ...
          if (data != null && data.isEmpty) {
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(
                builder: (context) => MembershipScreen(mobile:mobilenumberController.text),
              ),
            );
            setSnackbarForLogin(scaffoldMessengerKey, context, msg!);
          ...
          body: _isNetworkAvail
              ? Form(
                  ...
                  child: Stack(
                    children: [
                      Container(
                        padding: EdgeInsets.only(bottom: MediaQuery.of(context).padding.bottom, left: 15, right: 15, top: height / 17.0),
                        child: SingleChildScrollView(
                          child: Column(
                            children: [
