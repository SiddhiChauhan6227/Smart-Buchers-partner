...
import 'package:erestro/Screen/manage_promocode.dart';
import 'package:erestro/Screen/profile.dart';
...
import 'Offers/create_edit_offers.dart';
import 'add_product.dart' as add;
import 'editProduct.dart' as edit;
import 'package:http_parser/http_parser.dart';

class Media extends StatefulWidget {
  final from, pos, type,banner;
  const Media({Key? key, this.from, this.pos, this.type,this.banner}) : super(key: key);
  @override
...
class _MediaState extends State<Media> with TickerProviderStateMixin {
      ...
      if (scrollLoadmore) {
        ...
        try {
          ...
          http.Response response = await http
              .post(getMediaApi,
                  body: parameter, headers: await ApiUtils.getHeaders())
              .timeout(
                const Duration(
                  seconds: timeOut,
                ),
  ...
  getMediaItem(int index) {
    return Card(
      child: InkWell(
        onTap: () {
          log("hjmk ${widget.from}");
          log("hjmkrdds  ${widget.type}");
          print("hjmk jhbgds ");
          setState(() {
            mediaList[index].isSelected = !mediaList[index].isSelected;
            // In Media class when a media item is selected:
            // In the Media class, modify the part where image is selected:

// For offer image (when banner is false)
            if (widget.from == "offer" && !widget.banner) {
              if (widget.type == "add" || widget.type == "edit") {
                Map<String, String> result = {
                  "path": "${mediaList[index].subDic!}${mediaList[index].name!}",
                  "url": mediaList[index].image!,
                  "relativePath": mediaList[index].path!
                };
                Navigator.pop(context, result);
              }
            }
// For banner image (when banner is true)
            else if (widget.from == "offer" && widget.banner) {
              if (widget.type == "add" || widget.type == "edit") {
                Map<String, String> result = {
                  "path": "${mediaList[index].subDic!}${mediaList[index].name!}",
                  "url": mediaList[index].image!,
            ...
            if (widget.from == "main") {
              if (widget.type == "add") {
                ...
                print("rftgbhunyjmk,l ${add.productImage}");
              }
              if (widget.type == "edit") {
                edit.productImage =
                    "${mediaList[index].subDic!}${mediaList[index].name!}";
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                            '${getTranslated(context, "Size")!} : ${mediaList[index].size!}'),
                        Text(
