...
import 'dart:convert';
import 'dart:math';
import 'package:erestro/Helper/apiUtils.dart';
import 'package:erestro/Screen/MyTransactions.dart';
import 'package:erestro/Screen/membership.dart';
import 'package:erestro/Screen/promocodeList.dart';
import 'package:erestro/Screen/tags.dart';
...
import '../main.dart';
import 'Offers/offers_lists.dart';
...
//============================= Build Method ===================================
  ...
  Widget build(BuildContext context) {
    print("drfgyhnjmk $isOfferAllowed");
...
//========================= getStatics API =====================================
  ...
  Future<void> getSetting() async {
    ...
    apiBaseHelper.postAPICall(getSettingsApi, parameter, context).then(
      (getdata) async {
       ...
       String stripePublishableKey =  getdata["data"]["payment_method"][0]['stripe_publishable_key'];
       String stripeSecretKey =  getdata["data"]["payment_method"][0]['stripe_secret_key'];
       String stripeCurrencyCode =  getdata["data"]["payment_method"][0]['stripe_currency_code'];

       print("fegufeLKU${getdata["data"]["payment_method"]}");
       print("fegufeLKU waew ${getdata["data"]["payment_method"][0]['stripe_secret_key']}");
       print("fegufeLKU$stripeSecretKey \n $stripePublishableKey");

          SharedPreferences prefs = await SharedPreferences.getInstance();
          prefs.setString("stripeSecretKey", stripeSecretKey);
          prefs.setString("stripePublishableKey", stripePublishableKey);
          prefs.setString("stripeCurrencyCode", stripeCurrencyCode);
  ...
  Future<void> updateRestaurantStatus(String status) async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        Response response = await post(updateRestaurantTimeAPI,
  ...
  Future<void> getPlans() async {
    ...
    if (_isNetworkAvail) {
      try {
        isJwtToken = await getPrefrence(token);
        var parameter = {};
        ...
        apiBaseHelper
            ...
            .then((getdata) async {
          print("sxdcfgvbhnjmk,l ");
          bool error = getdata["error"];
          String msg = getdata["message"];
          print("dcfvgbhnj ${getdata["data"][0]['allow_offers'].runtimeType}");
          // print("dcfvgbhnj ${getdata["data"][0]}");
          if (!error) {
            isOfferAllowed = getdata["data"][0]['allow_offers'];
            setState(() {});
          } else {
            setsnackbar(msg, context);
          }
      ...
      apiBaseHelper
        ...
        (getdata) async {
          ...
          if (!error) {
            ...
            partnerWishPromocode =
                data[0]["permissions"]["partner_wise_promocode"];
        ...
        child: Container(
          ...
          child: ListView(
            ...
            children: <Widget>[
              ...
              if (isOfferAllowed == "1")
                _getDrawerItem(
                    16, getTranslated(context, "offers")!, Icons.local_offer),
              const Divider(),
              _getDrawerItem(3, getTranslated(context, "WALLETHISTORY")!,
                  Icons.account_balance_wallet_outlined),
              _getDrawerItem(11, getTranslated(context, "My Transactions")!,
              ...
              partnerWishPromocode == "1"
                  ? _getDrawerItem(14, getTranslated(context, "promoCode")!,
                      Icons.discount_outlined)
                  : const SizedBox.shrink(),
              _getDrawerItem(13, getTranslated(context, "Add Attributes")!,
                  Icons.edit_attributes_outlined),
              _getDrawerItem(
                  12, getTranslated(context, "Tags")!, Icons.style_outlined),
              _getDrawerItem(15, getTranslated(context, "plans")!,
                  Icons.next_plan_outlined),
              const Divider(),
              _getDrawerItem(14, getTranslated(context, "deleteAccount")!,
                  Icons.delete_outline),
              _getDrawerItem(5, getTranslated(context, "ChangeLanguage")!,
                  Icons.translate),
              _getDrawerItem(6, getTranslated(context, "Terms & Conditions")!,
                  Icons.speaker_notes_outlined),
              const Divider(),
              _getDrawerItem(7, getTranslated(context, "PRIVACYPOLICY")!,
                  Icons.lock_outline),
              _getDrawerItem(9, getTranslated(context, "CONTACTUS")!,
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: curDrwSel == index
            ? const LinearGradient(
                ...
                colors: [Color(0xffdd4a48), Color(0xffdd4a48)],
                stops: [0, 1],
              )
            : null,
      ...
      child: ListTile(
        ...
        title: Text(
          title,
          style: TextStyle(
              color: curDrwSel == index ? white : black, fontSize: 15),
        ),
        onTap: () {
          if (title == getTranslated(context, "HOME")!) {
          ...
          } else if (title == getTranslated(context, "offers")!) {
            setState(
              () {
                curDrwSel = index;
              },
          ...
          } else if (title == getTranslated(context, "plans")!) {
            setState(
              () {
                curDrwSel = index;
              },
            ...
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => MembershipScreen(mobile: ""),
              ),
            );
                                        ...
                                        child: AnimatedAlign(
                                          ...
                                          child: Padding(
                                            padding: EdgeInsets.all(
                                                globalRestaurantTimeStatus ==
                                                        "1"
                                                    ? 2.0
                                                    : 5.0),
                                            child: globalRestaurantTimeStatus ==
                                                    "1"
